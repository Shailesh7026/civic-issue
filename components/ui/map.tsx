...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Intentionally not logging the full error to avoid console spam 
      // (webglcontextcreationerror is expected in environments without hardware acceleration)
      setInitError("Failed to initialize map. WebGL may not be supported by your browser or hardware acceleration is disabled.");
  ...
  return (
    <MapContext.Provider value={contextValue}>
      <div
        ref={containerRef}
        className={cn("relative h-full w-full bg-muted/20 flex items-center justify-center", className)}
      >
        {initError ? (
          <div className="text-center p-6 bg-background rounded-lg border shadow-sm max-w-md m-4 flex flex-col items-center gap-3">
            <h3 className="font-semibold text-lg text-destructive">Map Initialization Failed</h3>
